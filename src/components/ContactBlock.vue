<template>
  <section class="contact column items-start">
    <div class="contact__miniTitle">
      <title-block :title="$t('contacts.title')"></title-block>
    </div>
    <header-block
    ...
    <div class="contact__body full-width row">
      <q-form class="full-width" @submit="onSubmit" @reset="onReset">
        <q-input
          name="name"
          v-model="name"
          :label="$t('contacts.inputs.name')"
          :dense="dense"
        />
        <q-input
          name="email"
          v-model="email"
          :label="$t('contacts.inputs.email')"
          :dense="dense"
        />
        <q-input
          name="message"
          v-model="message"
          :label="$t('contacts.inputs.message')"
          :dense="dense"
        />
        <div class="row justify-between items-baseline">
          ...
          <q-file
            class="col-6 col-md-2 col-sm-3 order-first order-sm-last q-mt-xs-md"
            standout
            color="purple-12"
            v-model="image"
          >
